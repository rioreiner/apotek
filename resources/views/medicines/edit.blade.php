                    ...
                    <form action="{{ route('medicines.update', $medicine) }}" method="POST" enctype="multipart/form-data">
                        ...
                        @method('PUT')
                         <div class="mb-4">
                            ...
                            <input type="file" name="image" value="{{ old('image', $medicine->image) }}" 
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="number" name="price" id="price" value="{{ old('price', $medicine->price) }}" step="0.01" min="0"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                @error('price')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <select name="category" id="category" 
                                ...
                                <option value="Obat Bebas Terbatas" {{ old('category', $medicine->category) == 'Obat Bebas Terbatas' ? 'selected' : '' }}>Obat Bebas Terbatas</option>
                                <option value="Obat Keras" {{ old('category', $medicine->category) == 'Obat Keras' ? 'selected' : '' }}>Obat Keras</option>
                                <option value="Obat Psikotropika" {{ old('category', $medicine->category) == 'Obat Psikotropika' ? 'selected' : '' }}>Obat Psikotropika</option>
                                <option value="Obat Narkotika" {{ old('category', $medicine->category) == 'Obat Narkotika' ? 'selected' : '' }}>Obat Narkotika</option>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="date" name="expired_date" id="expired_date" value="{{ old('expired_date', $medicine->expired_date->format('Y-m-d')) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                @error('expired_date')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
